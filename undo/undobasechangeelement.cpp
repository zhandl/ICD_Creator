...
UndoBaseChangeElement::UndoBaseChangeElement(QTreeWidget *theWidget, DomModel *newRegola, QList<int> newPath) : UndoCommand(theWidget, newRegola, newPath)
{
    _firstShot = true ;
    _element = NULL ;
...
void UndoBaseChangeElement::insertElement()
{
    if(NULL != _element) {
        ...
        if(!pathPos.isEmpty()) {
            parentElement = regola->findItemByArray(pathPos);
        }
