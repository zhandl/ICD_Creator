...
DomItem* SchemaSearchContextPosition::source()
{
    return _source ;
}


void SchemaSearchContextPosition::setSource(DomItem* value)
{
    _source = value ;
}
...
bool SchemaSearchContext::advanceSource()
{
    ...
    if(NULL != _parentElement) {
        ...
        while(!isEof()) {
            ...
            _currentIndex ++;
            DomItem *candidate = _parentElement->getChildAt(_currentIndex);
            if(NULL != candidate) {
                if(candidate->getType() == DomItem::ET_ELEMENT) {
...
void SchemaSearchContext::setTarget(DomItem *parent, DomItem *source, const int sourceIndex)
{
